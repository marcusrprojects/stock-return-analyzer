...
def calculate_cumulative_return_from_start(normalized_series: pd.Series) -> pd.Series:
    ...
    Returns:
        pd.Series: Series representing the cumulative return percentage. Returns empty Series on error.
    """
    if normalized_series.empty or normalized_series.isnull().all():
        return pd.Series(dtype=float)
    try:
        # The input is already normalized relative to the *overall* start
        # Cumulative return from the start of *this specific scope*
        first_val = normalized_series.iloc[0]
        if pd.isna(first_val) or first_val == 0:
            print("Warning: Invalid first value for cumulative return calculation.")
            return pd.Series(index=normalized_series.index, dtype=float) # Return NaNs
...
def plot_cumulative_returns(
    ...
    for i, (label, period_days) in enumerate(scopes.items()):
        ...
        if i >= smoothing_start_index:
            # Progressive smoothing: window increases for later charts
        ...
        if stock_ret.empty or bench_ret.empty:
            ...
            ax.set_title(f'{label}\n(No Data)', fontproperties=font_properties, color='red', fontsize=14)
    ...
    if plot_options['save']:
        ...
        if not save_path:
            ...
            output_dir = plot_options.get('output_dir', DEFAULT_OUTPUT_DIR)
            filename = f"{ticker}-vs-{benchmark_ticker}-CR-{date_str}.png"
...
def main():
    ...
    parser.add_argument("-o", "--output", type=str, default=None,
                        help="Output file path for the plot (e.g., 'plots/my_analysis.png'). "
                             "If not specified, a default filename is generated in 'data_viz/'.")
    ...
    # Add more arguments for plot customization if desired
    parser.add_argument("--stock-color", type=str, default='#004d80', help="Color for the stock line.")
    parser.add_argument("--bench-color", type=str, default='#80004d', help="Color for the benchmark line.")
    parser.add_argument("--font-family", type=str, default='Times New Roman', help="Font family for plots.")
    ...
    plot_options = {
        ...
        'grid': True, # kept internally, use show_grid for display control
        'linestyle': '-',
